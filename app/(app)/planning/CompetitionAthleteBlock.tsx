...
export default function CompetitionAthleteBlock({ block, athletes, sessionId, onAddDive }: Props) {
  ...
  // Kallas efter att ett hopp lagts till — refreshar listan
  const refreshDives = async (athleteId: string) => {
    const supabase = createClient()
    const { data } = await supabase
      .from('live_dive_log')
      .select('id, dive_code, dive_name, dd, status')
